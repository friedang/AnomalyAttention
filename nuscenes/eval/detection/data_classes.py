...
# Modified by Friedrich Dang, 2024.
...
class DetectionMetricData(MetricData):
    ...
    def __init__(self,
        ...
        self.fp = fp
        self.fn = fn
        self.binary_tp_count = binary_tp_count
...
class DetectionMetrics:
    ...
    def __init__(self, cfg: DetectionConfig):
        ...
        self._label_tps = defaultdict(lambda: defaultdict(float))
        self._label_fps = defaultdict(lambda: defaultdict(float))
        self._label_fns = defaultdict(lambda: defaultdict(float))
        self._label_max_recs = defaultdict(lambda: defaultdict(float))
        self._label_tp_errors = defaultdict(lambda: defaultdict(float))
        self.eval_time = None

    # TP because there's a different tp
    def add_label_TP(self, detection_name: str, dist_th: float, ap: float) -> None:
        self._label_tps[detection_name][dist_th] = ap

    def get_label_TP(self, detection_name: str, dist_th: float) -> float:
        return self._label_tps[detection_name][dist_th]    
    
    def add_label_fp(self, detection_name: str, dist_th: float, ap: float) -> None:
        self._label_fps[detection_name][dist_th] = ap

    def get_label_fp(self, detection_name: str, dist_th: float) -> float:
        return self._label_fps[detection_name][dist_th]

    def add_label_fn(self, detection_name: str, dist_th: float, ap: float) -> None:
        self._label_fns[detection_name][dist_th] = ap

    def get_label_fn(self, detection_name: str, dist_th: float) -> float:
        return self._label_fns[detection_name][dist_th]

    def add_label_max_rec(self, detection_name: str, dist_th: float, ap: float) -> None:
        self._label_max_recs[detection_name][dist_th] = ap

    def get_label_max_rec(self, detection_name: str, dist_th: float) -> float:
        return self._label_max_recs[detection_name][dist_th]
    ...
    @property
    def sum_tps(self) -> Dict[str, float]:
        sum_by_dist = {class_name: np.sum(list(d.values())) for class_name, d in self._label_tps.items()}
        return np.sum(list(sum_by_dist.values()))

    @property
    def sum_fps(self) -> Dict[str, float]:
        sum_by_dist = {class_name: np.sum(list(d.values())) for class_name, d in self._label_fps.items()}
        return np.sum(list(sum_by_dist.values()))

    @property
    def sum_fns(self) -> Dict[str, float]:
        sum_by_dist = {class_name: np.sum(list(d.values())) for class_name, d in self._label_fns.items()}
        return np.sum(list(sum_by_dist.values()))

    @property
    def mean_max_recs(self) -> Dict[str, float]:
        """ Calculates the mean over distance thresholds for each label. """
        mean_by_dist = {class_name: np.mean(list(d.values())) for class_name, d in self._label_max_recs.items()}
        return np.mean(list(mean_by_dist.values()))
    ...
    def serialize(self):
        return {
            ...
            'mean_max_recall': self.mean_max_recs,
            'TP': self.sum_tps,
            'FP': self.sum_fps,
            'FN': self.sum_fns,
